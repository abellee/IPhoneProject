...
@synthesize data, tableViewController;
...
- (id)initTableViewWithFrame:(CGRect)frame widthData:(EditableUITableViewData *)tableData scrollEnabled:(BOOL)scrollEnabled style:(UITableViewStyle)style
{
	...
	if(self){
		...
		tableViewController = [[RefreshTableViewController alloc] initWithStyle:style];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell == nil){
		...
		if(cellData.cellType == kTextField){
			...
			cell.selectionStyle = UITableViewCellSelectionStyleBlue;
		}else if(cellData.cellType == kView){
            cell.accessoryView = cellData.customView;
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
    }
    cell.textLabel.text = cellData.label;
    [cell.textLabel setTextColor:cellData.labelFontColor ? cellData.labelFontColor : [UIColor grayColor]];
    [cell.textLabel setFont:cellData.labelFont ? cellData.labelFont : [UIFont boldSystemFontOfSize:18.0]];
    return cell;
}
...
- (UITextField *)getTextField:(EditableUITableViewCellData *)cellData
{
	...
	textField.autocorrectionType = cellData.autocapitalizationType;
	textField.autocapitalizationType = cellData.autocapitalizationType;
    ...
    [textField setTextColor:cellData.fontColor ? cellData.fontColor : [UIColor grayColor]];
    [textField setFont:cellData.font ? cellData.font : [UIFont boldSystemFontOfSize:18.0]];
...
{
    ToggleableUITableViewCell* cell = (ToggleableUITableViewCell *)[textView superview];
	NSIndexPath* indexPath = [tableViewController.tableView indexPathForCell:cell];
	EditableUITableViewCellData* cellData = [data getDataByIndexPath:indexPath];
...
{
    EditableUITableViewCellData* cellData = [data getDataByIndexPath:indexPath];
    return cellData.cellHeight == 0 ? 40 : cellData.cellHeight;
}
